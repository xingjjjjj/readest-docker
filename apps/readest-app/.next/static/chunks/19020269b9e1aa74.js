...
`],{type:"text/css"})),m="data-foliate-id",g=async e=>{let n={},i=await p(e),l=new u(i),a=e=>[...i.querySelectorAll(e)],g=e=>{let t=r(e.querySelector("nickname"));if(t)return t;let n=r(e.querySelector("first-name")),i=r(e.querySelector("middle-name")),o=r(e.querySelector("last-name"));return{name:[n,i,o].filter(e=>e).join(" "),sortAs:o?[o,[n,i].filter(e=>e).join(" ")].join(", "):null}},f=e=>e?.getAttribute("value")??r(e),b=i.querySelector("title-info annotation");if(n.metadata={title:r(i.querySelector("title-info book-title")),identifier:r(i.querySelector("document-info id")),language:r(i.querySelector("title-info lang")),author:a("title-info author").map(g),translator:a("title-info translator").map(g),contributor:a("document-info author").map(g).concat(a("document-info program-used").map(r)).map(e=>Object.assign("string"==typeof e?{name:e}:e,{role:"bkp"})),publisher:r(i.querySelector("publish-info publisher")),published:f(i.querySelector("title-info date")),modified:f(i.querySelector("document-info date")),description:b?l.convert(b,{annotation:["div",s]}).innerHTML:null,subject:a("title-info genre").map(r)},i.querySelector("coverpage image")){let e=l.getImageSrc(i.querySelector("coverpage image"));n.getCover=()=>fetch(e).then(e=>e.blob())}else n.getCover=()=>null;let h=Array.from(i.querySelectorAll("body"),e=>{let t=l.convert(e,{body:["body",c]});return[Array.from(t.children,e=>{let t=[e,...e.querySelectorAll("[id]")].map(e=>e.id);return{el:e,ids:t}}),t]}),y=[],x=h[0][0].map(({el:e,ids:t})=>({ids:t,titles:Array.from(e.querySelectorAll(":scope > section > .title"),(e,t)=>(e.setAttribute(m,t),{title:r(e),index:t})),el:e})).concat(h.slice(1).map(([e,t])=>{let r=e.map(e=>e.ids).flat();return t.classList.add("notesBodyType"),{ids:r,el:t,linear:"no"}})).map(({ids:e,titles:r,el:n,linear:i})=>{let l,a=(l=n.outerHTML,`<?xml version="1.0" encoding="utf-8"?>
<html xmlns="http://www.w3.org/1999/xhtml">
    <head><link href="${d}" rel="stylesheet" type="text/css"/></head>
    <body>${l}</body>
</html>`),s=new Blob([a],{type:o}),c=URL.createObjectURL(s);return y.push(c),{ids:e,title:t(n.querySelector(".title, .subtitle, p")?.textContent??(n.classList.contains("title")?n.textContent:"")),titles:r,load:()=>c,createDocument:()=>new DOMParser().parseFromString(a,o),size:s.size-Array.from(n.querySelectorAll("[src]"),e=>e.getAttribute("src")?.length??0).reduce((e,t)=>e+t,0),linear:i}}),S=new Map;return n.sections=x.map((e,t)=>{let{ids:r,load:n,createDocument:i,size:o,linear:l}=e;for(let e of r)e&&S.set(e,t);return{id:t,load:n,createDocument:i,size:o,linear:l}}),n.toc=x.map(({title:e,titles:t},r)=>{let n=r.toString();return{label:e,href:n,subitems:t?.length?t.map(({title:e,index:t})=>({label:e,href:`${n}#${t}`})):null}}).filter(e=>e),n.resolveHref=e=>{let[t,r]=e.split("#");return t?{index:Number(t),anchor:e=>e.querySelector(`[${m}="${r}"]`)}:{index:S.get(r),anchor:e=>e.getElementById(r)}},n.splitTOCHref=e=>e?.split("#")?.map(e=>Number(e))??[],n.getTOCFragment=(e,t)=>e.querySelector(`[${m}="${t}"]`),n.destroy=()=>{for(let e of y)URL.revokeObjectURL(e)},n};e.s(["makeFB2",0,g])}]);