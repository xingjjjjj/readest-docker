...
"[project]/packages/foliate-js/uri-template.js [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const replace = (str, map)=>str.replace(regex, (_, operator, variableList)=>{
        ...
        return first + values.map(([name, value])=>value ? named ? name + (value ? '=' + value : ifemp) : value : '').filter((x)=>x).join(sep);
    });
const getVariables = (str)=>new Set(Array.from(str.matchAll(regex), ([, , variableList])=>variableList.split(',').map((varspec)=>varspec.match(varspecRegex)?.[1])).flat());
...
//# sourceMappingURL=packages_foliate-js_uri-template_dde92481.js.map