...
"[project]/packages/foliate-js/fb2.js [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class FB2Converter {
    constructor(fb2){
        ...
        this.bins = new Map(Array.from(this.fb2.getElementsByTagName('binary'), (el)=>[
                el.id,
                el
            ]));
    }
...
const makeFB2 = async (blob)=>{
    ...
    book.metadata = {
        ...
        contributor: $$('document-info author').map(getPerson)// techincially the program probably shouldn't get the `bkp` role
        // but it has been so used by calibre, so ¯\_(ツ)_/¯
        .concat($$('document-info program-used').map(getElementText)).map((x)=>Object.assign(typeof x === 'string' ? {
                name: x
            } : x, {
                role: 'bkp'
            })),
    ...
    const urls = [];
    const sectionData = bodyData[0][0]// make a separate section for each section in the first body
    .map(({ el, ids })=>{
    ...
    .concat(bodyData.slice(1).map(([sections, body])=>{
        ...
        urls.push(url);
        const title = normalizeWhitespace(el.querySelector('.title, .subtitle, p')?.textContent ?? (el.classList.contains('title') ? el.textContent : ''));
        return {
            ids,
            title,
            titles,
...
//# sourceMappingURL=packages_foliate-js_fb2_7cdbed03.js.map