...
"[project]/packages/foliate-js/mobi.js [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const huffcdic = async (mobi, loadRecord)=>{
    ...
    const table1 = Array.from({
        length: 256
    }, (_, i)=>offset1 + i * 4).map((offset)=>getUint(huffRecord.slice(offset, offset + 4))).map((x)=>[
            x & 0b1000_0000,
            x & 0b1_1111,
            x >>> 8
        ]);
...
class PDB {
    ...
    async open(file) {
        ...
        }, (_, i)=>getUint(buffer.slice(i * 8, i * 8 + 4))).map((x, i, a)=>[
                x,
                a[i + 1]
            ]);
    }
...
class MOBI6 {
    ...
    async init() {
        ...
        this.#sections = [
            0
        ].concat(Array.from(str.matchAll(mbpPagebreakRegex), (m)=>m.index)).map((start, i, a)=>{
    ...
    async loadText(section) {
        ...
        const fileposList = this.#fileposList.filter(({ number })=>number >= section.start && number < section.end).map((obj)=>({
                ...obj,
...
class KF8 {
    ...
    async init() {
        ...
        const resources = await this.getResourcesByMagic([
            'RESC',
            'PAGE'
        ]);
        ...
        try {
            ...
            const map = ({ label, pos, children })=>{
                ...
                if (arr) arr.push(off);
                else this.#fragmentOffsets.set(fid, [
                    off
                ]);
                return {
        ...
        this.rendition = {
            ...
            viewport: Object.fromEntries(exth.originalResolution?.split('x')?.slice(0, 2)?.map((x, i)=>[
                    i ? 'height' : 'width',
                    x
                ]) ?? [])
        };
    ...
    async loadResourceBlob(str) {
        ...
        const result = [
            ...
            MIME.CSS,
            MIME.SVG
        ].includes(type) ? await this.replaceResources(this.mobi.decode(raw)) : raw;
        const detail = {
            data: result,
            type
        };
...
//# sourceMappingURL=packages_foliate-js_mobi_93aecbb3.js.map