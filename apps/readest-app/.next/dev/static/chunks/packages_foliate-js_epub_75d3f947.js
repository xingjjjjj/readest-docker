...
"[project]/packages/foliate-js/epub.js [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const getMetadata = (opf)=>{
    ...
    for (const [keys, val] of [].concat(dc.creator?.map(makeContributor)?.map(remapContributor('author')) ?? [], dc.contributor?.map(makeContributor)?.map(remapContributor('contributor')) ?? []))for (const key of keys){
...
class Resources {
    constructor({ opf, resolveHref }){
        ...
        this.manifestById = new Map(this.manifest.map((item)=>[
                item.id,
                item
            ]));
        ...
        this.cover = this.getItemByProperty('cover-image') ?? this.getItemByID($$$(opf, 'meta').find(filterAttribute('name', 'cover'))?.getAttribute('content')) ?? this.manifest.find((item)=>item.id === 'cover' && item.mediaType.startsWith('image')) ?? this.manifest.find((item)=>item.href.includes('cover') && item.mediaType.startsWith('image')) ?? this.getItemByHref(this.guide?.find((ref)=>ref.type.includes('cover'))?.href) ?? this.manifest.find((item)=>item.mediaType.startsWith('image'));
        this.cfis = __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$foliate$2d$js$2f$epubcfi$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["fromElements"]($$itemref);
    }
    getItemByID(id) {
    ...
    resolveCFI(cfi) {
        const parts = __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$foliate$2d$js$2f$epubcfi$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["parse"](cfi);
        const top = (parts.parent ?? parts).shift();
        let $itemref = __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$foliate$2d$js$2f$epubcfi$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["toElement"](this.opf, top);
        ...
        if ($itemref && $itemref.nodeName !== 'idref') {
            top.at(-1).id = null;
            $itemref = __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$foliate$2d$js$2f$epubcfi$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["toElement"](this.opf, top);
        }
        const idref = $itemref?.getAttribute('idref');
        const index = this.spine.findIndex((item)=>item.idref === idref);
        const anchor = (doc)=>__TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$foliate$2d$js$2f$epubcfi$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["toRange"](doc, parts);
        return {
            index,
            anchor
        };
...
class Loader {
    ...
    async createURL(href, data, type, parent) {
        ...
        if (newType === MIME.XHTML) {
            this.#cacheXHTMLContent.set(url, {
                href,
                type: newType,
                data: newData
            });
    ...
    async loadReplaced(item, parents = []) {
        ...
        if ([
            ...
            for (const el of doc.querySelectorAll('[srcset]'))el.setAttribute('srcset', await replaceSeries(el.getAttribute('srcset'), /(\s*)(.+?)\s*((?:\s[\d.]+[wx])+\s*(?:,|$)|,\s+|$)/g, (_, p1, p2, p3)=>this.loadHref(p2, href, parents).then((p2)=>`${p1}${p2}${p3}`)));
...
var _c;
__turbopack_context__.k.register(_c, "WebCryptoSHA1");
...
//# sourceMappingURL=packages_foliate-js_epub_75d3f947.js.map