...
"[project]/packages/foliate-js/paginator.js [client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class View {
    ...
    scrolled({ marginTop, marginRight, marginBottom, marginLeft, gap, columnWidth }) {
        ...
        setStylesImportant(doc.documentElement, {
            ...
            'padding': vertical ? `${marginTop * 1.5}px ${marginRight}px ${marginBottom * 1.5}px ${marginLeft}px` : `${marginTop}px ${gap / 2 + marginRight / 2}px ${marginBottom}px ${gap / 2 + marginLeft / 2}px`,
            'column-width': 'auto',
            'height': 'auto',
            'width': 'auto',
    ...
    setImageSize() {
        ...
        for (const el of doc.body.querySelectorAll('img, svg, video')){
            ...
            setStylesImportant(el, {
                'max-height': vertical ? maxHeight !== 'none' && maxHeight !== '0px' ? maxHeight : '100%' : `${height - marginTop - marginBottom}px`,
                'max-width': vertical ? `${width - marginLeft - marginRight}px` : maxWidth !== 'none' && maxWidth !== '0px' ? maxWidth : '100%',
                'object-fit': 'contain',
...
class Paginator extends HTMLElement {
    ...
    constructor(){
        ...
        this.addEventListener('load', ({ detail: { doc } })=>{
            ...
            doc.addEventListener('selectionchange', ()=>{
                ...
                if (!isPointerSelecting && isPointerSelecting && sel.type === 'Range') checkPointerSelection(range, sel);
    ...
    open(book) {
        ...
        book.transformTarget?.addEventListener('data', ({ detail })=>{
            ...
            detail.data = Promise.resolve(detail.data).then((data)=>data// unprefix as most of the props are (only) supported unprefixed
                .replace(/([{\s;])-epub-/gi, '$1')// `page-break-*` unsupported in columns; replace with `column-break-*`
    ...
    #beforeRender({ vertical, rtl, background }) {
        ...
        const columnWidth = vertical ? size / divisor - marginTop * 1.5 - marginBottom * 1.5 : size / divisor - gap - marginRight / 2 - marginLeft / 2;
        this.setAttribute('dir', rtl ? 'rtl' : 'ltr');
        // set background to `doc` background
        // this is needed because the iframe does not fill the whole element
        this.columnCount = divisor;
        this.#replaceBackground(background, this.columnCount);
        const marginalDivisor = vertical ? Math.min(2, Math.ceil(Math.floor(width) / Math.floor(maxInlineSize))) : divisor;
        const marginalStyle = {
    ...
    snap(vx, vy) {
        ...
        const page = Math.floor(Math.max(min, Math.min(max, (start + end) / 2 + (isNaN(d) ? 0 : d * 2))) / size);
    ...
    async #scrollTo(offset, reason, smooth) {
        ...
        if ((reason === 'snap' || smooth) && this.hasAttribute('animated') && !this.hasAttribute('eink')) return animate(this.containerPosition, offset, 300, easeOutQuad, (x)=>this.containerPosition = x).then(()=>{
            this.#scrollBounds = [
                offset,
                this.atStart ? 0 : size,
                this.atEnd ? 0 : size
    ...
    async #scrollToAnchor(anchor, reason = 'anchor') {
        ...
        if (rects) {
            ...
            const rect = Array.from(rects).find((r)=>r.width > 0 && r.height > 0 && r.x >= 0 && r.y >= 0) || rects[0];
            if (!rect) return;
    ...
    #getVisibleRange() {
        if (this.scrolled) return getVisibleRange(this.#view.document, this.start, this.end, this.#getRectMapper());
    ...
    async #goTo({ index, anchor, select }) {
        if (index === this.#index) await this.#display({
            ...
            await this.#display(Promise.resolve(this.sections[index].load()).then(async (src)=>{
...
//# sourceMappingURL=packages_foliate-js_paginator_d198bf99.js.map